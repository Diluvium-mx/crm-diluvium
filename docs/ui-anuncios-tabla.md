...
base; recibe filas `AdRow` y las pinta. **Conectada** (25-sep-2026, rama `feat/anuncios-meta`): es la
sección `/anuncios` (`app/(app)/anuncios/page.tsx` arma las filas con `listAdsForPeriod` de
`lib/ads/queries.ts`; reglas de conteo en docs/anuncios.md › Interfaz). La vista previa con datos de
ejemplo (`/vista-previa/anuncios`) y la lista vieja se quitaron.
...
| `status` | `"active" \| "paused" \| "unknown"` | Punto de estado ● Activa / Pausada; `unknown` (Meta no respondió o aún no se consulta) = "—". |
...
- **Filtro** Activas / Todas (por defecto Activas). "Activas" esconde solo las **pausadas**: las de
  estado "—" siguen a la vista (una falla de Meta no vacía la tabla).
...
## Qué hace el bloque de Anuncios (hecho)

1. Arma `AdRow[]` para el periodo (clientes = contactos con clic del anuncio en el periodo;
   compraron = de esos, los que hoy están en etapa `compra`; `linkClicks` queda `null` hasta
   Métricas de anuncios). Sin nombres de Meta: campaña "—", conjunto vacío.
2. `/anuncios` = `<AdsTable rows={…} />` en un contenedor de alto fijo (`h-[calc(100dvh-4rem)]`).
