...
//   · Grok 4.6: https://docs.x.ai/docs/models (2 / 6, caché 0.50; desde 200 mil
//     tokens de entrada 4 / 12, caché 1).
//   · Qwen 3.7 Flash por OpenRouter: https://openrouter.ai/api/v1/models (0.03 /
...
// `tiers`: precio que aplica desde `minInputTokens` de entrada total (ordenados de
// menor a mayor). Sin tramo que aplique, el precio base.
export type ModelPrice = TierPrice & { tiers?: readonly (TierPrice & { minInputTokens: number })[] };
...
type BasePrice = {
  ...
  cacheRead?: number;
  cacheWrite?: number;
  tiers?: readonly { minInputTokens: number; input: number; output: number; cacheRead: number; cacheWrite: number }[];
};
...
export const DEFAULT_MODEL_PRICES: Readonly<Record<string, BasePrice | null>> = {
  ...
  "qwen-3.7-flash": {
    ...
    cacheWrite: 0.038,
    tiers: [
      { minInputTokens: 32_000, input: 0.1, output: 0.4, cacheRead: 0.02, cacheWrite: 0.125 },
      { minInputTokens: 256_000, input: 0.2, output: 0.8, cacheRead: 0.04, cacheWrite: 0.25 },
    ],
  },
};
    ...
    ...(base.tiers
      ? {
          tiers: base.tiers.map((t) => ({
            ...
            inputPerMTok: t.input,
            outputPerMTok: t.output,
            cacheReadPerMTok: t.cacheRead,
            cacheWritePerMTok: t.cacheWrite,
          })),
        }
      : {}),
  };
...
export function computeCostUsd(usage: ModelUsage, price: ModelPrice | null): number | null {
  ...
  // Tramo por entrada total (el último cuyo mínimo se alcanzó); si no, el base.
  const tier = [...(price.tiers ?? [])].reverse().find((t) => total >= t.minInputTokens);
