...
try {
  ...
  finish({ pageCount, png: Buffer.from(png).toString("base64") });
} catch (error) {
  finish({ error: error instanceof Error ? error.message : String(error) });
}

// stdout a un pipe es ASÍNCRONO: salir con process.exit() justo después de
// write() puede truncar la salida (Node lo documenta). Se sale en el callback,
// cuando todo se entregó.
